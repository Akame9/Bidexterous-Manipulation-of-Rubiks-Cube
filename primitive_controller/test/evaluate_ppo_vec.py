"""
Evaluation script for trained PPO Vec agent on bidexhands Rubik's cube manipulation.
This script loads a trained ppo_vec.py model and evaluates it with the MuJoCo viewer.
"""
...
from primitive_controller.ppo_vec import PPOAgentVec, get_device, print_gpu_info
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Evaluate trained PPO Vec agent on Rubik\'s Cube environment')
    
    # Model arguments
    parser.add_argument('--model_path', type=str, default='saved_models/ppo_vec_model_best.pth',
    ...
    # Create PPO Vec agent
    print("\nCreating PPO Vec agent...")
    agent = PPOAgentVec(
        state_dim=state_dim,
    ...
    if hasattr(agent.policy, '_orig_mod'):
        agent.policy._orig_mod.eval()
    else:
        agent.policy.eval()
